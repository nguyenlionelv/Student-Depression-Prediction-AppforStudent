...
FEATURE_COMMENTS = {
    ...
    "suicidal_thoughts": {
        "increases": "Suy nghĩ tiêu cực là dấu hiệu cần được chú ý quan trọng. Đặc biệt, nếu bạn có (hoặc từng có) ý nghĩ tự tử, hãy lập tức tìm kiếm sự giúp đỡ từ chuyên gia sức khỏe tâm thần.",
...
RECOMMENDATIONS = {
    "high_risk": [
        ...
        " Gọi đường dây hỗ trợ sức khỏe tâm thần: 0909 658 035 (miễn phí, 24/7).",
...
def translate_factors(shap_values, feature_names: List[str], top_k: int = 5):
    ...
    """
    from src.deploy.schemas import FactorDetail
    import numpy as np
...
def build_explanation(
    ...
    # Recommendations
    recs = []
    if risk_level in ("Rất cao"):
        recs += RECOMMENDATIONS["high_risk"]
    elif risk_level in ("Có nguy cơ", "Ít khả năng", "Tương đối cao"):
